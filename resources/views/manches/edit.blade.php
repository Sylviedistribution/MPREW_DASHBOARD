...
@section('contents')
    <div class="offset-3 col-md-6">
        <div class="card" style="background-color: #b3d4fc">
            <h3 class="text-center">MODIFIER DES MANCHES</h3>
            <div class="card-body offset-2">
                <form method="POST" action="{{ route('manches.update',$manche) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-8">
                        <div class="form-group">
                            <label for="nom" class="form-control-label">Nom</label>
                            <input class="form-control @error('nom') is-invalid @enderror" name="nom" type="text"
                                   onfocus="focused(this)" onfocusout="defocused(this)" value="{{ old('nom', $manche->nom) }}">
                            @error('nom')
                    ...
                    <div class="col-md-8">
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" id="description"
                                      name="description" placeholder="Décrivez votre col...">{{ old('description', $manche->description) }}"</textarea>
                            @error('description')
                            <span class="invalid-feedback">{{ $message }}</span>
                            @enderror
                        </div>
